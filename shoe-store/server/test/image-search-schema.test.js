...
test('image embedding schema defines the table, constraints, and vector index', () => {
  ...
  assert.match(table[1], /product_id BIGINT NOT NULL/);
  assert.match(table[1], /product_image_id BIGINT NOT NULL/);
  ...
  assert.match(table[1], /CONSTRAINT product_image_embeddings_product_image_product_fk\s+FOREIGN KEY \(product_image_id, product_id\) REFERENCES product_images\(id, product_id\) ON DELETE CASCADE/);
  ...
  try {
    ...
    await assert.rejects(
      client.query(
        `INSERT INTO product_image_embeddings (product_id, product_image_id, embedding_model, status)
         VALUES ($1, $2, $3, 'active')`,
        [productId, imageId, `${model}-active-null`]
      )
    );
    await assert.rejects(
      client.query(
    ...
    cleanupProductIds.push(ownershipProductId);
    ...
    assert.equal(ownershipConstraint.rows.length, 1);
    assert.match(ownershipConstraint.rows[0].definition, /FOREIGN KEY \(product_image_id, product_id\)/);
  } finally {
