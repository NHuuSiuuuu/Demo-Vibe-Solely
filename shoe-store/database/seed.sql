...
SELECT p.id, data.sku, data.size, data.color, data.stock_quantity, data.price_delta
FROM products p
JOIN (
  VALUES
...
) AS data(slug, sku, size, color, stock_quantity, price_delta) ON p.slug = data.slug;
